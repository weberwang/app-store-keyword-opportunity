...
export function computeMarketInsight(keyword, topApps) {
    if (!topApps.length) {
        return {
    ...
    const hints = [];
    const qualityGapApps = topApps.filter((app) => app.reviews >= 500 && app.score > 0 && app.score < 3.8);
    const qualityGap = qualityGapApps.length > 0;
    if (qualityGap) {
        const worstScore = Math.min(...qualityGapApps.map((a) => a.score)).toFixed(1);
        hints.push(`质量洼地：${qualityGapApps.length} 款现有 App 评分低于 3.8（最低 ${worstScore}★）但下载量可观，` +
    ...
    if (staleRatio >= 0.5) {
        hints.push(`市场陈旧：前 ${topApps.length} 名中有 ${staleApps.length} 款（${Math.round(staleRatio * 100)}%）超过 1 年未更新，` +
    ...
    if (topDev && topDev[1] >= 3) {
        hints.push(`大厂占位：「${topDev[0]}」在搜索结果中占据 ${topDev[1]} 个位置，` +
            `市场存在强势玩家——差异化定位（细分人群或功能点）比正面竞争更有效。`);
    }
    else if (uniqueDevs >= topApps.length * 0.8) {
        hints.push(`分散市场：前 ${topApps.length} 名由 ${uniqueDevs} 个不同开发者瓜分，无明显龙头——` +
    ...
    if (freeRatio === 1) {
        hints.push(`全免费格局：前 ${topApps.length} 名均为免费 App，付费专区尚属空白——` +
            `如果功能价值高，直接以付费或订阅切入，用户付费意愿未经验证但竞争少。`);
    }
    else if (freeRatio < 0.3) {
        const avgPrice = (paidApps.reduce((sum, a) => sum + a.price, 0) / paidApps.length).toFixed(2);
        hints.push(`付费主导格局：${paidApps.length} 款主流 App 为付费模式，平均定价 $${avgPrice}——` +
    ...
    if (has17plus >= 2) {
        hints.push(`成人内容聚集：多款竞品内容评级为 17+，若你做全年龄段版本，可在分类搜索中独占差异化位置。`);
    }
    else if (allKidFriendly) {
        hints.push(`全年龄/儿童向市场：竞品评级均为 4+，家长和教育机构是主要目标用户，家庭订阅是合理变现方向。`);
    }
    if (paidApps.length > 0 && freeRatio > 0.3 && freeRatio < 0.7) {
        const avgPaid = paidApps.reduce((s, a) => s + a.price, 0) / paidApps.length;
        if (avgPaid > 4) {
            hints.push(`高价空缺：市面付费 App 平均定价 $${avgPaid.toFixed(2)}，$1.99-$2.99 价格带可能存在空缺——` +
                `低价切入更易获取首批用户和评价。`);
        }
    }
    if (!hints.length) {
        hints.push(`市场格局较均衡，建议深入阅读头部 App 的差评（1-2 星评论），` +
    ...
    const summary = {
        ...
        avgScore: topApps.length
            ? parseFloat((topApps.reduce((s, a) => s + a.score, 0) / topApps.length).toFixed(2))
            : 0,
        uniqueDevs,
...
export function buildProductStrategy(keywords) {
    if (!keywords?.length)
        return null;
    const buckets = {
        qualityGap: [],
    ...
    if (avgScore > 0 && avgScore < 4.0) {
        positioningHints.push(`**体验优先**：竞品平均评分 ${avgScore.toFixed(1)} 分偏低，重点投入流畅度和核心流程，目标上架后评分稳定在 4.5+。`);
    }
    else {
        positioningHints.push(`**功能差异化**：竞品整体评分尚可（${avgScore.toFixed(1)}），靠体验难以脱颖而出，需在功能维度找到竞品尚未覆盖的场景（查阅竞品 1-2 星差评）。`);
    }
    const topOpportunity = [...keywords].sort((a, b) => b.opportunityScore - a.opportunityScore)[0];
    if (topOpportunity) {
        const seeds = topOpportunity.seeds || [];
        positioningHints.push(`**最优切入词**：「${topOpportunity.term}」（机会分 ${topOpportunity.opportunityScore}，竞争分 ${topOpportunity.competitionScore}）——以此为核心功能命名，利于 ASO 自然搜索。`);
        if (seeds.length) {
            positioningHints.push(`**种子市场**：从「${seeds.slice(0, 2).join("」「")}」这类搜索词的用户出发，理解他们的场景与痛点，MVP 只做这一件事、做到极致。`);
        }
    }
    const allOpps = [
        ...buckets.qualityGap,
    ...
    return {
        ...
        meta: {
            ...
            avgOpportunityScore: Math.round(keywords.reduce((s, k) => s + k.opportunityScore, 0) / keywords.length),
