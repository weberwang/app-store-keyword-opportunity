...
async function fetchAppReviews(appId, { country = "us", pages = 2 } = {}) {
    const allReviews = [];
...
async function fetchAppRatings(appId, { country = "us" } = {}) {
    try {
        return await scraper.ratings({ id: String(appId), country });
    }
    catch {
        return null;
    }
}
function extractTopWords(texts, topN = 20) {
...
export async function analyzeCompetitorReviews(apps, { country = "us", pages = 2 } = {}) {
    const limit = pLimit(2);
    const results = await Promise.all(apps.map((app) => limit(async () => {
    ...
    for (const { word } of filteredPain.slice(0, 5)) {
        for (const { reviews } of results) {
            const hit = reviews.find((r) => r.score <= 2 && normalizeTerm(r.text || "").includes(word));
            if (hit) {
                painExamples.set(word, hit.text.slice(0, 100).replace(/\n/g, " ") + "…");
                break;
            }
        }
    }
    const appSummaries = results.map(({ app, reviews, ratingsData }) => {
        ...
        const avg = total > 0
            ? [1, 2, 3, 4, 5].reduce((s, star) => s + star * (hist[star] || 0), 0) / total
            : null;
        return {
            id: app.id,
            title: app.title,
...
//# sourceMappingURL=review-analysis.js.map