...
import { analyzeChartTrends, fetchChart, compareSnapshots, CHART_TYPES, } from "./lib/trends.js";
...
async function runCollect() {
    const answers = await inquirer.prompt([
        {
            ...
            validate: (v) => (v.trim() ? true : "至少输入一个种子词"),
        },
        {
            type: "list",
            name: "country",
        ...
        },
        {
            ...
            when: (ans) => ans.country === "__custom__",
    ...
    const country = answers.country === "__custom__"
    ...
    console.table(snapshot.keywords.slice(0, 10).map((item) => ({
        关键词: item.term,
    ...
    if (topInsights.length) {
        ...
        for (const item of topInsights) {
            const s = (item.insight.summary || {});
            console.log(`【${item.term}】 机会分 ${item.opportunityScore} | 竞品 ${s.appCount ?? "?"} 款 | 均分 ${s.avgScore ?? "?"} | 陈旧 ${s.staleCount ?? 0} 款`);
            for (const hint of item.insight.hints)
                console.log(`  → ${hint}`);
            console.log();
        }
    }
...
async function runQuery() {
    ...
    if (topInsights.length) {
        ...
        for (const item of topInsights) {
            const s = (item.insight.summary || {});
            console.log(`【${item.term}】 机会分 ${item.opportunityScore} | 竞品 ${s.appCount ?? "?"} 款（免费 ${s.freeCount ?? "?"}/${s.paidCount ?? "?"}付费）| 均分 ${s.avgScore ?? "?"}`);
            for (const hint of item.insight.hints)
                console.log(`  → ${hint}`);
            console.log();
        }
...
async function runTrend() {
    const answers = await inquirer.prompt([
        ...
        },
        {
            ...
            choices: Object.entries(CHART_TYPES).map(([value, info]) => ({
                name: `${info.label} 100`,
                value,
            })),
            default: ["top-free"],
            validate: (v) => (v.length ? true : "至少选择一个榜单"),
        },
        {
            type: "list",
            name: "country",
    ...
    console.log("\n[ 正在拉取 iTunes 实时榜单... ]\n");
    const chartResults = await Promise.all(answers.chartTypes.map(async (type) => {
        try {
            const apps = await fetchChart(answers.country, type, 100, answers.genre || "");
    ...
    console.log(`  合并后共 ${allApps.length} 款 App | 分析范围：${genreLabel}\n`);
    if (!allApps.length) {
    ...
    if (!answers.genre) {
        ...
        console.table(analysis.topCategories.map((c) => ({
            分类: c.category,
            上榜数量: c.count,
            占比: `${c.ratio}%`,
        })));
    ...
    console.table(analysis.topWords
        .slice(0, limit)
        .map((w) => ({
        热词: w.word,
        出现次数: w.count,
    ...
    if (analysis.topWords.length) {
        ...
        console.log(`参考种子词：${analysis.topWords
            .slice(0, 5)
            .map((w) => w.word)
            .join(", ")}\n`);
    }
...
async function runCompare() {
    ...
    let filtered = delta;
    if (genreFilter) {
        filtered = filtered.filter((d) => Array.isArray(d.topApps) &&
            d.topApps.some((a) => (a.genre || "").toLowerCase().includes(genreFilter.toLowerCase())));
    }
...
async function runStrategy() {
    ...
    if (genreName) {
        keywords = keywords.filter((kw) => Array.isArray(kw.topApps) &&
            kw.topApps.some((a) => (a.genre || "").toLowerCase().includes(genreName.toLowerCase())));
    }
    ...
    console.log(`\u2502  分析关键词 ${strategy.meta.totalKeywords} 个  |  平均机会分 ${strategy.meta.avgOpportunityScore}`);
    console.log("╰──────────────────────────────────────────╯\n");
    console.log("▼ 机会词盘点\n");
    const bucketLabels = {
    ...
    for (const [key, label] of Object.entries(bucketLabels)) {
        const items = strategy.opportunities[key];
        if (!items.length)
            continue;
        hasOpportunity = true;
        console.log(label);
        console.table(items.map((kw) => ({
            关键词: kw.term,
    ...
    console.log(`▼ 产品定位建议\n`);
    for (const hint of strategy.positioningHints)
        console.log(`  ${hint}`);
    console.log(`\n▼ 分阶段行动路线图\n`);
...
async function runReview() {
    ...
    if (answers.source === "snapshot") {
        ...
        const kw = (snapshot.keywords || []).find((k) => k.term.toLowerCase().includes(keyword) ||
    ...
    console.log(`  共获取 ${result.reviewsFetched} 条评论 | 数据时间：${result.analyzedAt.slice(0, 10)}\n`);
    console.log("── 竞品 App 评论概况 ──\n");
    console.table(result.appSummaries.map((a) => ({
    ...
    if (total > 0) {
        ...
        for (const star of [5, 4, 3, 2, 1]) {
            ...
            console.log(`  ${"★".repeat(star)}${"☆".repeat(5 - star)}  ${String(pct).padStart(3)}%  ${bar}  (${cnt.toLocaleString()})`);
        }
        console.log();
    }
    if (result.painPoints.length) {
        console.log("── 差评痛点词（用户最不满的方面）──\n");
        console.table(result.painPoints
            .slice(0, 10)
            .map((w) => ({
            痛点词: w.word,
            差评出现次数: w.count,
    ...
    if (result.sellingPoints.length) {
        ...
        console.table(result.sellingPoints
            .slice(0, 10)
...
// 多国市场对比
// ─────────────────────────────────────────────────────────────
async function runCountryCompare() {
    const answers = await inquirer.prompt([
        {
            type: "input",
            name: "terms",
    ...
    console.table(result.countrySummaries.map((c) => ({
    ...
    if (best)
        console.log(`  → 整体最适合备战的市场：「${best.country.toUpperCase()}」（平均机会分 ${best.avgOpportunity}）\n`);
    console.log("── 各关键词最佳市场──\n");
    for (const ts of result.termSummaries) {
        console.log(`【${ts.term}】→ 最佳国家：${ts.bestCountry.toUpperCase()} (机会分 ${ts.bestOpportunityScore})`);
        console.table(ts.countryResults.map((r) => ({
    ...
    while (true) {
        ...
        try {
            if (action === "collect")
                ...
                await runReview();
            else if (action === "country-compare")
                await runCountryCompare();
        }
        catch (e) {
            console.error(`\n错误：${e.message}\n`);
        }
