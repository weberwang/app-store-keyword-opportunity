...
export function registerStrategyTools(server) {
    server.tool("build_strategy", "从本地快照文件生成产品策略报告：机会词分桶、变现模式推荐、定位建议和分阶段行动路线图。", {
        file: z
            .string()
            .optional()
        ...
        try {
            ...
            const keywords = (snapshot.keywords || [])
                ...
                .slice(0, limit);
            if (!keywords.length) {
                return err(`快照中没有机会分 >= ${min_opportunity} 的关键词，请降低阈值或先运行 search_keywords 采集数据。`);
            }
            const strategy = buildProductStrategy(keywords);
            if (!strategy)
                return err("数据不足，无法生成策略报告。");
            return ok({
                snapshotFile,
                meta: {
                    ...strategy.meta,
