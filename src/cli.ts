...
async function runCollect(): Promise<void> {
  ...
  console.table(
    snapshot.keywords.slice(0, 10).map((item) => ({
      ...
      变现分: item.monetizationScore,
      缺口分: item.marketGapScore,
...
async function runQuery(): Promise<void> {
  const answers = await inquirer.prompt([
    ...
    {
      type: "input",
      name: "minMonetization",
      message: "最低变现潜力分 0-100（直接回车跳过）：",
    },
    {
      type: "input",
      name: "minMarketGap",
      message: "最低市场缺口分 0-100（直接回车跳过）：",
    },
    {
      type: "input",
  ...
  const results = queryKeywords(snapshot, {
    ...
    minMonetization: toFloatOrUndefined(answers.minMonetization),
    minMarketGap: toFloatOrUndefined(answers.minMarketGap),
